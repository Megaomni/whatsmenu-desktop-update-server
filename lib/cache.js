...
module.exports = class Cache {
  ...
  async cacheReleaseList(url) {
    ...
    for (let i = 0; i < matches.length; i += 1) {
      ...
      const nuPKG = url.replace(url.includes('RELEASES_ia32') ? 'RELEASES_ia32' : 'RELEASES_x64', matches[i])
  ...
  async refreshCache() {
    ...
    for (const asset of release.assets) {
      ...
      const releaseFiles = ['RELEASES', 'RELEASES_ia32', 'RELEASES_x64']
      if (releaseFiles.includes(name)) {
        try {
          if (!this.latest.files) {
            this.latest.files = {}
          }
          this.latest.files[name] = await this.cacheReleaseList(
