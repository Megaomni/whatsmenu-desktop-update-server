...
module.exports = ({ cache, config }) => {
  ...
  exports.download = async (req, res) => {
    ...
    if (userAgent.isMac && isUpdate) {
      ...
      if (userAgent.source.includes('x64')) {
        platform = 'exe_x64'
      } else {
        platform = 'exe_ia32'
      }
  ...
  exports.releases = async (req, res) => {
    ...
    if (userAgent.isWindows) {
      if (userAgent.source.includes('x64')) {
        content = latest.files.RELEASES_x64
      } else {
        content = latest.files.RELEASES_ia32
      }
    } else {
      content = latest.files.RELEASES_x64
    }
    console.log({ content, userAgent: req.headers['user-agent'] }, 'content');
